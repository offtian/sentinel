...
**Status:** complete (all 31 steps delivered across skills-runtime and graph-consumption-refactor branches)
**Created:** 2026-04-08
...
- [x] **Step 28: Graph consumption** — Completed via the `graph-consumption-refactor` plan. All three graph files (`sre_investigation.py`, `support_review.py`, `chart_generation.py`) now use `agent_for("name")` callable instead of module-level singletons. `Dependencies` dataclasses use `agent_for: Callable[[str], Any]` rather than individual model fields. Functional tests mock via `_build_fake_config()`. Residual `chart_parser_model`/`chart_generator_model` string params removed in `refactor/skills-runtime-completion` branch — model names now extracted from pre-built agents via `agent.model.model_name`.
- [x] **Step 29: Remove module-level `agent = build_agent()` fallback** — Completed via the `graph-consumption-refactor` plan. All 8 agent modules expose only `build_agent()` factories with no module-level singleton.
- [x] **Step 30: Remove the import-time `append_skills_to_prompt` / `render_skills_section` wiring** — `append_skills_to_prompt` removed and replaced by `compose_system_prompt` (config-driven, name-based). `render_skills_section` retained — it serves the dynamic runtime skill injection layer in `root_cause_analyser` and `response_drafter` and is not dead code.
- [x] **Step 31: Docs** — Plan documentation synced. INDEX.md updated. Outcome section filled in.
