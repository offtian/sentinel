    ...
    for attempt in range(max_retries + 1):
        ...
        try:
            files = await _generate_chart_files(
                spec=spec, policy=policy, model=model, error_context=error_context,
            )
        except Exception as exc:
            logs.log_exception(exc, params={"node": "GenerateChart", "attempt": generation_attempts})
            timings.append(pipeline_types.ChartStepTiming(
                step=f"GenerateChart (attempt {generation_attempts})", duration_ms=_elapsed_ms(gen_start),
            ))
        ...
        timings.append(pipeline_types.ChartStepTiming(
            step=f"ValidateChart (attempt {generation_attempts})", duration_ms=_elapsed_ms(val_start),
        ))
        ...
        logs.log_event(
            ...
            params={"service_name": spec.service_name, "attempt": generation_attempts, "errors": validation_result.errors},
        )
...
async def generate_chart(
    *,
    ...
    timings: list[pipeline_types.ChartStepTiming] = []
    reply_ctx = {"pipeline_start": pipeline_start, "timings": timings,
                 "parser_model": parser_model, "generator_model": generator_model}
    ...
    if not parser_model or not generator_model or max_retries is None:
        ...
        reply_ctx.update(parser_model=parser_model, generator_model=generator_model)
    ...
    try:
        ...
        timings.append(pipeline_types.ChartStepTiming(step="ParseRequest", duration_ms=_elapsed_ms(step_start)))
        return _make_reply(service_name="unknown", error=f"Failed to parse request: {exc}", **reply_ctx)
    timings.append(pipeline_types.ChartStepTiming(step="ParseRequest", duration_ms=_elapsed_ms(step_start)))
    ...
    try:
        ...
        timings.append(pipeline_types.ChartStepTiming(step="LoadPolicy", duration_ms=_elapsed_ms(step_start)))
        return _make_reply(service_name=spec.service_name, error=f"Policy not found: {exc}", **reply_ctx)
    timings.append(pipeline_types.ChartStepTiming(step="LoadPolicy", duration_ms=_elapsed_ms(step_start)))

    # Step 3: Merge spec with policy
    step_start = time.monotonic()
    merged_spec, violations = policies.merge_spec_with_policy(spec=spec, policy=policy)
    timings.append(pipeline_types.ChartStepTiming(step="MergeSpec", duration_ms=_elapsed_ms(step_start)))

    # Steps 4+5: Generate and validate (with self-heal loop)
    chart_output, validation_result, generation_attempts, last_error = await _generate_and_validate_loop(
        spec=merged_spec, policy=policy, violations=violations,
        model=generator_model, max_retries=max_retries, timings=timings,
    )

    if chart_output is None:
        return _make_reply(
            service_name=spec.service_name, validation_passed=False,
            policy_violations=len(violations), generation_attempts=generation_attempts,
    ...
    timings.append(pipeline_types.ChartStepTiming(step="ConfidenceScoring", duration_ms=_elapsed_ms(step_start)))
    ...
    timings.append(pipeline_types.ChartStepTiming(step="CommitToGitOps", duration_ms=_elapsed_ms(step_start)))

    return _make_reply(
        service_name=spec.service_name, files_generated=len(chart_output.files),
        validation_passed=True, policy_violations=len(violations),
        generation_attempts=generation_attempts, confidence=score, pr_url=pr_url,
        error=commit_error,
        **reply_ctx,
    )
