...
def _build_alert_classifier_evaluators() -> list[pydantic_evals.evaluators.Evaluator]:
    ...
    Checks: severity match, category match, and non-empty summary.
    """
    return [
...
def _build_root_cause_evaluators(
    *, case: dict[str, Any]
    ...
    Checks: keyword coverage, has remediation, has evidence, confidence threshold.
    """
...
def _build_response_drafter_evaluators(
    *, case: dict[str, Any]
    ...
    Checks: non-empty response, has sources, keyword coverage.
    """
    expected = case["expected"]
    return [
        structural.StructuralCheck(
            ...
            check_type="non_empty",
            rubric="Response text is non-empty",
        ),
        structural.StructuralCheck(
...
def load_cases(
    *,
    ...
    :param agent_name: One of "alert_classifier", "root_cause_analyser", "response_drafter".
